...
sealed class LazyLightClassDataHolder(
    ...
    override fun findData(findDelegateIfAny: (PsiJavaFileStub) -> PsiClass?): LightClassData? =
        LazyLightClassData { stub ->
            findDelegateIfAny(stub)
        }
    ...
    class ForClass(
        ...
        override fun findDataForClassOrObject(classOrObject: KtClassOrObject): LightClassData =
            LazyLightClassData { stub ->
                stub.findDelegateIfAny(classOrObject)
            }
    }
    ...
    private inner class LazyLightClassData(
        findDelegateIfAny: (PsiJavaFileStub) -> PsiClass?
    ) : LightClassData {
        override val clsDelegate: PsiClass? by lazyPub { findDelegateIfAny(javaFileStub) }

        private val dummyDelegate: PsiClass? by lazyPub { inexactStub?.let(findDelegateIfAny) }
        ...
        override fun getOwnMethods(containingClass: KtLightClass): List<KtLightMethod> {
            return dummyDelegate?.methods?.map { dummyMethod ->
                ...
                KtLightMethodImpl.lazy(dummyMethod, containingClass, methodOrigin) {
                    ...
                    */
                    val exactDelegateMethod = clsDelegate?.findMethodsByName(dummyMethod.name, false)
                        ?.firstOrNull(byMemberIndex) ?: clsDelegate?.methods?.firstOrNull(byMemberIndex)
